...
//       // Add item to cart
//       addToCart: async (product, size, quantity,token) => {
//         try {
...
//                 item => item.product_id === product.id && item.sizeId === size.id
//               );
...
//           if (token) {
//             // If logged in, try API first
//             const response = await apiRemoveFromCart(token, itemId);
						...
						try {
							...
							set({
								...
								error:
									...
									"Failed to remove item from cart on server, using local cart",
							});
						}
					} else {
						// If not logged in, use local state
