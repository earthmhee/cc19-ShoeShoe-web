...
//                     const imageArray = item.product && item.product.images ? parseImages(item.product.images) : [];
//                     const mainImage = imageArray[0] || "";
...
//                             </div>
//                             <div className="ml-4">
//                               <div className="text-sm font-medium text-gray-900">{item.product?.productname || 'Product'}</div>
...
//                 <div>
//                   <div className="text-sm text-gray-500">Address</div>
//                   <div className="font-medium">{order.customer?.address || 'N/A'}</div>
//                 </div>
//               </div>
//             </div>
//           </div>
...
const OrderDetail = () => {
  ...
  // Extract address from customer object, which may be formatted differently based on backend
  const customerAddress = order.customer?.address 
    ? (typeof order.customer.address === 'string' 
      ? order.customer.address 
      : `${order.customer.address.homenum || ''}, ${order.customer.address.subdistrict || ''}, ${order.customer.address.district || ''}, ${order.customer.address.province || ''}, ${order.customer.address.postcode || ''}`.replace(/,\s,/g, ',').replace(/^,\s|\s,$/g, ''))
    : 'N/A';
