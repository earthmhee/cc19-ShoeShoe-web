...
function ResponsiveSidebar() {
  ...
  return (
    <div className="flex h-screen">
      ...
      {/* Sidebar - visible when isOpen=true on mobile or always on large screens */}
      <div 
        className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition duration-200 ease-in-out z-30 w-64 bg-white shadow-lg overflow-y-auto`}
        aria-hidden={!isOpen}
      >
      ...
      {/* Main content */}
      <div className={`flex-1 p-4 bg-gray-100 transition-all duration-200 ${isOpen ? 'lg:ml-64' : 'ml-0'}`}>
        <div className="max-w-7xl mx-auto">
          {/* Header with toggle button for larger screens */}
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <button 
                className="hidden lg:block p-2 mr-4 rounded-md text-gray-700 hover:bg-gray-200 transition-colors"
                onClick={toggleSidebar}
                aria-label="Toggle sidebar"
              >
                {isOpen ? (
              ...
              <h1 className="text-2xl font-bold">CARNIVAL®</h1>
            </div>
